...
void proximity(int point_id, std::vector<bool>& processed, const std::vector<std::vector<float>> points, 
  std::vector<int>& cluster, KdTree* tree, float distanceTol){
	  processed[point_id] = true;
	  cluster.push_back(point_id);

	  std::vector<int> ids = tree->search(points[point_id], distanceTol);

	  for(auto id:ids){
		  if(!processed[id])
		  	proximity(id, processed, points, cluster, tree, distanceTol);
	  }
}
...
std::vector<std::vector<int>> euclideanCluster(const std::vector<std::vector<float>>& points, KdTree* tree, float distanceTol)
{
	...
	for(int i = 0; i<points.size(); i++){
		...
		proximity(i, processed, points, cluster, tree, distanceTol);
